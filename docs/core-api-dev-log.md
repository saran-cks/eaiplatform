...
## Session 25: close the cross-tenant leak on the `/observability` read-side (DD-25) — 2026-07-01
*   **Trigger**: a review flagged the #1 finding across all three reviewers — `GET /observability/traces|evals|datasets` call `_get_scope()` only for the 401, then **discard** the scope; no `tenant_id` reaches the use-case or port. Reading the adapter confirmed the blast radius: Phoenix uses **one global project** (`self._project = settings.otel_service_name`, `phoenix/client.py`) shared by all tenants, and traces carry query text + retrieved-chunk **content** — so any valid JWT read every tenant's data verbatim, no id-guessing. `/drift` was the only correctly-scoped route (it already passed `scope.tenant_id`).
*   **Fix**: made tenant isolation a **port-contract property**, not adapter etiquette. `ObservabilityPort.get_traces/get_evals/get_datasets` + `curate_dataset` now take `tenant_id: str` as a **required kwarg** (no default → an unscoped read is a type error). Routes pass `scope.tenant_id`; the use-case (`get_phoenix_data.py`) threads it through. Phoenix adapter enforces + **fails closed**: traces/evals filter on the `tenant.id` span attribute already stamped by chat/agent-runtime/MCP-connector (untagged span → excluded, not leaked); `get_evals` scopes span ids *before* requesting annotations so a cross-tenant span id is never queried; datasets get identity via name — `curate_dataset` namespaces to `t:{tenant}:{name}`, `get_datasets` returns only that prefix, stripped. NoOp adapter signatures updated to match.
*   **Design choice (why the port, not the route)**: filtering after an unscoped port read would still pull every tenant's data into process and lets a future adapter (Langfuse, etc.) forget the filter; putting `tenant_id` in the signature makes "unscoped read" unrepresentable for all backends. Rejected optional `tenant_id=None`-means-all (one forgotten arg reopens the leak) and per-tenant Phoenix projects (infra/exporter change, doesn't retro-protect existing shared-project data — attribute-filtering secures the read path today and composes with per-tenant projects later). Full rationale in **DD-25**.
*   **Test** (`test_observability.py`, +5): fake Phoenix client (`_FakePhoenix` with `get_spans`/`get_span_annotations`/`datasets.list`/`add_examples_to_dataset`, injected via `adapter._client` to bypass the lazy real-client build) — tenant A never sees B's spans; a **shared** `session_id` can't cross the tenant boundary; `get_evals` only queries the caller's span ids; datasets prefix-filtered + stripped; `curate_dataset` asserts `t:{tenant}:` namespacing. Fixed the two existing NoOp/fail-soft tests to pass `tenant_id`.
*   **Issues Faced & Resolved**: mypy — net-new errors **zero**, confirmed by stashing (baseline 384; the only delta in non-test source is a line-number shift of a pre-existing `span`-method annotation error, 66→69). The +11 total-count delta is entirely the new test functions, which follow the repo's existing unannotated-test convention.
*   **Verification**: `uv run pytest src/tests -q` → **169 passed**; `uv run ruff check src` clean. **DD-25** added. Live cross-tenant read isolation against a real Phoenix is deferred as **ST-6** (added PENDING to smoke-tests).

---

## Session 26: close the cross-tenant DoS on `POST /agent/{id}/interrupt` — 2026-07-01
*   **Trigger**: a review flagged that `interrupt_agent` calls `_get_scope()` only for the 401 and then **discards** the scope, dispatching `container.agent.interrupt(agent_session_id=...)` straight to the runner. `AgentPort.interrupt` takes no scope and `LangGraphRunner` keys its interrupt set/`_tasks` map **purely off `agent_session_id`** — so any authenticated principal, including one from another tenant, could cancel any running agent by id. Cross-tenant DoS (no data read, but an attacker with a valid JWT tears down a victim tenant's in-flight agent runs). Exactly parallel to the `/observability` scope-dropped leak (Session 25/DD-25), and the fix template sat two functions away: `list_artifacts`/`get_artifact` already thread `scope.tenant_id` into a tenant-scoped store read.
*   **Fix**: routed the interrupt through a new **`InterruptAgentUseCase`** (`core/use_cases/agent/interrupt_agent.py`) that resolves the session **tenant-scoped** first — `store.get_agent_session(agent_session_id, tenant_id)` (already `WHERE agent_session_id=$1 AND tenant_id=$2` in the Postgres adapter) — and dispatches `agent.interrupt` **iff** the caller's tenant owns it. A miss returns `False` → the route raises **404** (indistinguishable from "does not exist" — we never confirm another tenant's session id). The route now builds the use-case from `container.store`+`container.agent`, keeping the runner-facing call out of the router and matching the artifacts pattern. The `AgentPort.interrupt` signature is unchanged: ownership is authorized *before* the runner is touched, at the use-case seam where the scope still exists — the runner stays a dumb by-id canceller. The in-request client-disconnect path (`run_agent`'s `GeneratorExit → container.agent.interrupt`) is unaffected: it cancels its *own* just-started session under the same request's already-validated scope, so no lookup is needed.
*   **Design note (no new DD)**: this applies the DD-25 principle ("a scope validated for 401 must actually reach the authorization decision") to the agent lifecycle rather than introducing a new one. Ownership granularity matches the established template — **tenant** (what `get_agent_session`/artifacts enforce). Within-tenant subject-level interrupt (user A cancelling user B's run in the same tenant) is **not** guarded here because the store lookup is tenant-keyed and that's the codebase's current ownership unit; noted as a possible future tightening (would need a `subject_id` filter on the lookup), not a DoS across the trust boundary the review flagged.
*   **Test** (`test_agent_mock.py`, +2 = 8): `test_interrupt_owned_session_dispatches` (owned session → lookup with the caller's tenant, then `agent.interrupt` awaited once) and `test_interrupt_foreign_tenant_is_denied` (tenant-scoped lookup returns `None` → use-case returns `False`, `agent.interrupt` **never** awaited — the cross-tenant path). `test_architecture.py`/`test_pdp_chokepoint.py` stay green (new use-case imports only ports + domain; no new `.call_tool(` site).
*   **Issues Faced & Resolved**: `mypy src/api/routes/agent.py` reports 4 errors (`_get_scope` unused-ignore + missing return annos on `_get_scope`/`run_agent`/`_event_generator`) — confirmed **pre-existing** by stashing the change (identical 4 on HEAD, only line numbers shifted by the added import). Net-new mypy errors from this session: **zero**; the new `interrupt_agent.py` + the interrupt handler type-clean.
*   **Verification**: `uv run pytest src/tests -q` → **171 passed** (169 + 2); `uv run ruff check` clean on all changed files. No smoke test — the authorization decision is fully exercised by the unit tests over the store contract; no new live-service behavior beyond the existing agent-run E2E.
