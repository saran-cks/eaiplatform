...
|------|------------------|-----------------|-------|
| `AvScannerPort` ✅ *adapter built (offline); live = ST-2* | clamd client | a running **clamav** daemon (freshclam 3–6h refresh) | INSTREAM scan of raw bytes; the static magic-byte/size checks are already pure in `security_gate.py`. **Done:** `adapters/av_scanner/clamd.py` — sync `clamd` client via `asyncio.to_thread`, fail-closed (`AvScannerUnavailable`→`scan_error` quarantine), tested offline incl. a fake INSTREAM TCP server (DD-22). Remaining: DI wiring + live daemon check (ST-2). |
...
| `VectorSinkPort` ✅ *adapter built (offline); live = ST-2* | Qdrant writer | a **Qdrant** instance | upsert points (id = `chunk_id`) with named `dense`+`sparse` vectors + payload; delete by id. **Keep `wait=True`** (read-your-writes on ACK, DD-20 addendum); `# FUTURE EXTENSION` read `consistency`/write `ordering` when Qdrant goes multi-node. **Done:** `adapters/sink/qdrant.py` — bootstraps the collection to the contract, `wait=True`, tested offline on `qdrant-client :memory:`. **Also fixed DD-23:** `chunk_id` is now a UUIDv5 (Qdrant rejects a raw sha256 point id). Remaining: DI wiring + live check (ST-2). |
