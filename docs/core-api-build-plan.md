...
- [x] **`ValkeyAdapter` is fail-soft (Session 24, DD-24)** — every method swallows a backend `RedisError` and returns the miss sentinel (read→`None`/`False`/`0`, write/evict→no-op), so a Valkey outage degrades chat/session/risk to cache-miss instead of 500ing the request path. Centralized in the adapter (not the ~15 call sites, which already treat a miss as "fall back to the authoritative source"). Covered by `test_cache_valkey.py`.
